...
@router.get("/surfplan")
def surf_groups_for_surfplan(day: date, session: Session = Depends(get_db)):
    surf_plan_service = SurfPlanService(
        SQLAlchemySurfPlanRepositoryImpl(session),
        StudentService(SQLAlchemyStudentRepositoryImpl(session)),
        TideServiceMockImpl())

    surfgroups = surf_plan_service.generate_surf_groups_for_week(day)
    print(surfgroups)

    initialGroups = [Group(level="Beginner", age_group="Adults", students=surfgroups["beginner"]),
                     Group(level="Beginner Plus", age_group="Adults", students=surfgroups["beginner_plus"]),
                     Group(level="Intermediate", age_group="Adults", students=surfgroups["intermediate"]),
                     Group(level="Advanced", age_group="Adults", students=surfgroups["advanced"]),
                     Group(level="Teens", age_group="Teens", students=surfgroups["teens"]),
                     Group(level="Kids", age_group="Kids", students=surfgroups["kids"])
                     ]
    slots = [Slot(datetime.now(), initialGroups), Slot(datetime.now(), [])]
    return SurfPlan(plan_date=day, slots=slots, non_participating_guests=surfgroups["non_participating_guests"])
    ...
    def export_students_to_excel(sunday: date, session: Session = Depends(get_db)):
        ...
        response = create_excel_week_overview(sunday, surf_groups)
        return response
